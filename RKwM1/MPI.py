...
def simulation(z0, depth ,Nb):
    ...
    Z_record = np.array([z]) # recording main steip position
    T_record = np.array([0.0]) # recording main steip time
    W_record = np.array([0.0]) # recording Weiner process for main step
...
def Euler_Maruyama(z0, depth, Nb, T, W):
    ...
    for i in range(nt-1):
        ...
        Temp_z = np.where(Temp_z > H, 2*H-Temp_z, Temp_z)
        z[i+1] = Temp_z
    hist, _ = np.histogram(z, bins=np.linspace(0, depth, Nb))
    return hist
...
z0_array = np.random.uniform(0, 10, Np)
for i in range(Np):
    z0 = z0_array[i] 
    
    temp_RS, T, W = simulation(z0, depth, Nb)
    temp_RS = temp_RS/T.size
    hist_RS = hist_RS + temp_RS
    
    temp_EM = Euler_Maruyama(z0, depth, Nb, T, W)
    temp_EM = temp_EM/T.size
    hist_EM = temp_EM + hist_EM
...
if rank == 0:
    hist_RS_global = np.zeros_like(hist_RS)
    hist_EM_global = np.zeros_like(hist_EM)
# Make a dummy on all other ranks
else:
    hist_RS_global = None
    hist_EM_global = None
# First argument is sendbuf, second is recvbuf
comm.Reduce(hist_RS, hist_RS_global, op=MPI.SUM, root=0)
comm.Reduce(hist_EM, hist_EM_global, op=MPI.SUM, root=0)

# Store the results on rank 0 only
bins = np.linspace(0, depth, Nb)
mid = bins[0:-1] + (bins[1] - bins[0])/2
if rank == 0:
    np.save('hist_RS.npy', hist_RS_global)
    np.save('hist_EM.npy', hist_EM_global)
    np.save("mid.npy", mid)

#%%
#z0 = np.random.uniform(0, 10)
#Z, T, W, hist = simulation(z0 ,1, 500)
